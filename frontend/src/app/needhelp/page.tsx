...
const CATEGORIES: Category[] = [
  ...
  { key: "transporte", title: "Transporte", tone: "blue" },
  { key: "escuela", title: "Escuela", tone: "orange" },
  { key: "un_amigo", title: "Un amigo", tone: "neutral" },
  { key: "doctor", title: "Doctor", tone: "green" },
  { key: "telefono_movil", title: "Teléfono móvil", tone: "blue" },
  { key: "trabajo", title: "Trabajo", tone: "orange" },
  { key: "cuenta_bancaria", title: "Cuenta bancaria", tone: "green" },
  { key: "ropa_muebles", title: "Ropa y muebles", tone: "blue" },
...
export default function AyudaPage() {
  ...
  const [familyDetails, setFamilyDetails] = useState<{ age: string; sex: string }[]>(
    []
  );
  ...
  const suggestedMessage = useMemo(() => {
    const parts: string[] = [];
    const categories = selectedCategories.map(cat => cat.title).join(", ");
    const familyCount = Number.parseInt(familyMembers, 10);
    const familyInfo = Number.isFinite(familyCount) && familyCount > 0 ? familyCount : null;
    const familyDetailsText =
      familyDetails.length > 0
        ? familyDetails
            .map((member, index) => {
              const ageValue = Number.parseInt(member.age, 10);
              const hasAge = Number.isFinite(ageValue);
              const gender = member.sex;
              let role = "persona";

              if (hasAge && gender === "femenino") {
                if (ageValue < 10) role = "niña";
                else if (ageValue < 18) role = "jovencita";
                else if (ageValue < 60) role = "adulta";
                else role = "idosa";
              } else if (hasAge && gender === "masculino") {
                if (ageValue < 10) role = "niño";
                else if (ageValue < 18) role = "jovencito";
                else if (ageValue < 60) role = "adulto";
                else role = "idoso";
              } else if (gender === "otro") {
                role = hasAge && ageValue < 18 ? "adolescente" : "persona";
              }

              if (hasAge) {
                return `${role} de ${ageValue} años`;
              }
                ...
                gender === "femenino"
                  ...
                  : gender === "masculino"
                    ...
                    : gender === "otro"
                      ? "persona"
                      : "persona";
              return `${sexFallback} (edad no indicada)`;
            })
            .join(", ")
        : "";
    const englishText =
    ...
    parts.push("Hola,");
    if (categories) {
      parts.push(`estoy buscando apoyo en ${categories}.`);
    } else {
      parts.push("estoy buscando apoyo.");
    }
    if (age) {
      parts.push(`Tengo ${age} años`);
    }
    if (sex) {
      const sexLabel =
        sex === "femenino"
          ...
          : sex === "masculino"
            ...
            : sex === "otro"
              ? "persona no binaria"
              : "prefiero no decir mi sexo";
      if (sex === "prefiero_no_decir") {
        parts.push("y prefiero no decir mi sexo.");
      } else if (age) {
        parts[parts.length - 1] = `${parts[parts.length - 1]}, soy ${sexLabel}.`;
      } else {
        parts.push(`Soy ${sexLabel}.`);
      }
    } else if (age) {
      parts[parts.length - 1] = `${parts[parts.length - 1]}.`;
    }
    if (nativeCountry) {
      parts.push(`Soy de ${nativeCountry}.`);
    }
    if (city) {
      parts.push(`Estoy en ${city}.`);
    }
    if (occupation) {
      parts.push(`Mi ocupación es ${occupation}.`);
    }
    if (familyInfo) {
      const familyIntro =
        familyInfo === 1
          ? "Mi familia está compuesta por una persona"
          : `Mi familia está compuesta por ${familyInfo} personas`;
      parts.push(`${familyIntro}${familyDetailsText ? ":" : "."}`);
    }
    if (familyDetailsText) {
      parts.push(`${familyDetailsText}.`);
    }
    if (arrivalDate) {
      parts.push(`Llegamos el ${arrivalDate}.`);
    }
    if (englishText) {
      parts.push(englishText);
    }
    parts.push(
      "Agradecería mucho cualquier orientación, acompañamiento o recursos que puedan ofrecernos. Gracias por su tiempo y apoyo."
    );
  ...
  return (
    <div className="w-full">
      ...
      <p className="mt-4 max-w-3xl text-base text-[#1b3f7a]/70">
        Doce opciones esenciales para empezar con el pie derecho.
        No solicitamos información sobre inmigración ni documentos; tu privacidad y
        ...
        <form onSubmit={submit} className="mt-8 grid gap-6">
          ...
          <div className="grid gap-6 md:grid-cols-4">
            <label className="block text-sm text-[#1b3f7a]">
              ...
              <input
                ...
                className="mt-2 w-full rounded-2xl border border-[#f1d0ae] bg-[#fff6ec] px-3 py-2 text-xs text-[#12376c] outline-none ring-2 ring-transparent transition focus:ring-[#1aa1d5]/40"
                placeholder="Ej: Ama de casa, estudiante, mecánico"
              />
            </label>
            <label className="block text-sm text-[#1b3f7a]">
              Ciudad
            ...
            <label className="block text-sm text-[#1b3f7a]">
              ...
              <select
                value={speaksEnglish}
                onChange={e => setSpeaksEnglish(e.target.value)}
                className="mt-2 w-full rounded-2xl border border-[#f1d0ae] bg-[#fff6ec] px-3 py-2 text-xs text-[#12376c] outline-none ring-2 ring-transparent transition focus:ring-[#1aa1d5]/40"
              >
                <option value="">Selecciona</option>
                <option value="si">Sí</option>
                <option value="no">No</option>
                <option value="un_poco">Un poco</option>
          ...
          {familyDetails.length > 0 ? (
            <div className="space-y-3">
              <div className="text-xs font-semibold uppercase tracking-[0.25em] text-[#1b3f7a]/60">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {familyDetails.map((member, index) => (
                  <div
                    key={`family-${index}`}
                    className="rounded-2xl border border-[#f4d3b2] bg-[#fff6ec] p-3"
                  >
                    <div className="text-xs font-semibold text-[#12376c]">
                      Persona {index + 1}
                    </div>
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      <label className="block text-xs text-[#1b3f7a]">
                        Edad
                        <input
                          value={member.age}
                          onChange={e =>
                          ...
                          className="mt-1.5 w-full rounded-xl border border-[#f1d0ae] bg-white px-2.5 py-1.5 text-xs text-[#12376c] outline-none ring-2 ring-transparent transition focus:ring-[#1aa1d5]/40"
                          placeholder="Ej: 12"
                          inputMode="numeric"
                        />
                      </label>
                      <label className="block text-xs text-[#1b3f7a]">
                        ...
                        <select
                          ...
                          onChange={e =>
                            setFamilyDetails(prev => {
                              const next = [...prev];
                              next[index] = { ...next[index], sex: e.target.value };
                              return next;
                            })
                          }
                          className="mt-1.5 w-full rounded-xl border border-[#f1d0ae] bg-white px-2.5 py-1.5 text-xs text-[#12376c] outline-none ring-2 ring-transparent transition focus:ring-[#1aa1d5]/40"
          ...
          <div className="rounded-2xl border border-[#f4d3b2] bg-[#fff6ec] p-4 text-sm text-[#1b3f7a]/75">
            <div className="text-[11px] font-semibold uppercase tracking-[0.25em] text-[#1b3f7a]/60">
              Sugerencia de mensaje
            </div>
            <p className="mt-2 text-sm text-[#12376c]">{suggestedMessage}</p>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <button
                type="button"
                onClick={() => {
                  setDetails(suggestedMessage);
                  setUseSuggestedMessage(true);
                }}
                className="rounded-full border border-[#f4d3b2] bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#12376c] transition hover:-translate-y-0.5 hover:border-[#f28c28]"
              >
                Usar mensaje sugerido
              </button>
              {useSuggestedMessage ? (
                <span className="text-xs text-[#1b3f7a]/65">
